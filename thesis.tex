...
When people have to enter expenses, this involves retrieving data from one or more sources in the environment and entering it into a computer system. Switching between entering data and looking up the required data can be disruptive, as people have to pay a time cost each time they resume their data entry task. Furthermore, the cost to access these sources may vary, which can further influence the disruptiveness of looking up the information. In order to design interfaces that support people in their data entry work, it is therefore important to understand how these differences in access affect switching behaviour between entering and retrieving data. 
This thesis investigates how information access costs (IAC) affect how people manage subtasks of looking up information for a data entry task. Six studies are carried out to understand the impact of IAC in the context of entering expenses in a finance office setting.

Study 1 aimed to explore the type of data entry tasks in financial offices at two universities, and the context in which these tasks are done. A prevalent data entry task is processing expenses from university staff and students, which involves retrieving information from multiple sources with different IACs. People often have to go in and out of the data entry system to look up information, and upon resuming the entry task it is difficult to see where they are in the task.  Errors are common and therefore data has to be entered and checked by multiple people, before it can be submitted to the system. People batch their work to do their data entry tasks in one sequence, which suggests people make attempts to minimise switching between tasks.

Study 2 aimed to explore whether the effects of IAC on copying from one source, as found in previous studies, transfer to copying numbers. In a controlled experiment the cost of accessing information in a copying task was manipulated. The study showed that when access costs from one source increase, people make fewer switches between entering data and checking this source to minimise time costs. This made people more efficient but slightly increased errors, though error rate was low overall.

Study 3 aims to further understand the expenses task, the information that office workers need for this task, where they need to get it from, and the tools and strategies they use to retrieve the information and enter it into a computer. Office workers dealing with processing expenses are shadowed doing their work and interviewed to explain their activities. They are also asked to demonstrate the task while thinking aloud.

Study 4 aims to investigate to what extent observed behaviour in Study 3 is due to the used information sources and its access costs.  A controlled experiment manipulates the cost of accessing information when using multiple sources in an expenses task, to investigate to what extent strategies are influenced by differences in IAC. The expected outcome is that people will optimise the task as they become familiar and experienced in the task: initially they will look up each data item the moment they need it, but as they become familiar with the IAC of sources, they first finish entering all information from low IAC sources, before looking up information of high IAC sources. The identified behaviour of how people manage looking up information is used to create a set of design recommendations for the current expenses system. 

Study 5 aims to evaluate these design recommendations in a controlled experiment, to see if making design changes influence the strategies people adopt, and can make people adopt more accurate and/or efficient strategies.

Study 6 aims to evaluate the design recommendations in situ with finance employees, to ascertain how appropriate the proposed recommendations are in the context in which they would be used.

The primary contribution of this thesis is to show the fragmented nature of an expenses task, and that the IAC of required information sources affects how people manage how they look up certain information, which in turns impacts accuracy and efficiency. This finding has implications for the design of the current expenses system. A second contribution is demonstrating of how changing certain design features can potentially better support people in managing these subtasks of looking up information for a data entry task.
...
\fbox{\begin{minipage}[5pt]{\textwidth}
...
Instead of making design changes to the interface, \citet{LiSimon2015} investigated if people can be motivated to check by incentives. 
...
\chapter{Study 1: Understanding data entry tasks in a financial office}\label{ch:Study1}
...
\subsection{Interviewing procedure}
...
\subsection{Importance of accuracy and paper trails}
\begin{figure}[!ht]
\centering
...
\subsection{Information sources and access costs}
...
\subsection{Batching data entry tasks}
...
\subsection{Multiple checking stages}
...
\chapter{Study 2: The effect of IAC on copying numbers}\label{ch:Study2}
\fbox{\begin{minipage}{\textwidth}
\subsubsection{Chapter outline}
In this chapter, I describe a lab study that replicates the Blocks World Task study using coloured as well as numbered blocks. The purpose of this study was to test whether the effect of IAC on a copying task, as found in previous studies, extends to copying numbers.
A part of the data was collected by Katherine Corneilson, an affiliate undergraduate student at UCL. I designed and implemented the study, collected the remainder of the data, and conducted the analyses.
\end{minipage}}

 

\section{Introduction}
Study 1 illustrated that people had to retrieve and copy data from multiple sources with varying information access costs (IAC). People were not always able to look at the data source while entering data, and information sometimes had to be briefly kept in memory before people could enter it.
...
\subsection{Global task performance}
...
\subsection{The effect of block type on task strategies and performance}
...
\subsection{The effect of IAC on task strategies and performance}
...
\subsection{Summary}
The purpose of this study was to study the effect of IAC on strategy, speed and accuracy, and investigate if the effect of IAC, as found in previous experimental studies, would extend to copying numbers. In order to be able to compare results of this study with previous IAC studies the same task paradigm of the Blocks World Task was used.
...
First, the input method matters. An input method that is sufficiently slow may make people accurate, regardless of the level of IAC. 
...
In order to be able to further conduct experiments that study how people can best be supported when retrieving information for a data entry task, it is important to understand where they have to get it from. Therefore, for the next study I will focus on the expenses task identified in Study 1 and look at the resources people need for this task, where they need to get it from, and how costly it is to access this information.

\chapter{Study 3: Information sources for entering expenses}
\fbox{\begin{minipage}{\textwidth}
\subsubsection{Chapter outline}
In this chapter, I give an outline for a planned study. The aim of this study is to investigate the information sources finance office workers need for an expenses task, and how they currently manage subtasks of looking up information from an expenses task.
\end{minipage}}
...
For the expenses task observed in Study 1, people have to enter different types of information, these do not all come from the same source, and the sources can have a different IACs. For example, some information can be recalled from memory, some has to be typed from a paper sheet, some numbers have to be calculated first, and some information has to be looked up in an electronic information system. 
...
My data collection will be informed by two methodological approaches: contextual inquiry and think-aloud.
...
Initially, the intended method of this study was to conduct a contextual inquiry, followed by a week-long diary study where office workers would make a diary entry of their expenses tasks. Participants would submit a diary entry either by writing down a description or by taking a photograph of the task setting, showing the information sources. At the end of the day, they would be asked answer the following questions about their short entries: what information did you need, where did you need to get it from, and when did you look up the information. This method built on a study by \citet{Sohn2008}, where participants kept a diary of their mobile information needs and how they addressed those needs. 
...
\chapter{Study 4: the effect of IAC on looking up information}
...
\subsection{Aim}
This study aims to investigate in a controlled experiment to what extent people's strategies in switching between entering and looking up information from multiple sources is influenced by changes in IAC, and whether different strategies affect performance.

The office setting of Study 1 and 3 will be simulated in a laboratory environment. Participants will have to retrieve data from a number of sources, and enter this into a desktop computer. The sources are on paper, on a second screen, or on the same screen as where the participant has to enter the data. I will use keylogging data to measure whether people interrupt entering data, presumably to retrieve data. Keylog intervals of more than > is considered to be an interruption. To supplement quantitative measures, I will video record and/or observe them and code the timing and frequency of visits people make to the different information sources.
...
\begin{itemize}
\item 
Participants will be slower to resume the entry task after looking up information from high IAC sources than information from low IAC sources.
\item  
as IAC increases, people try to memorise and copy over more data from one source in one go.
\item 
People will initially look up items at the moment in the task they need it.
\item 
...
\chapter{Study 5: evaluating design changes in a lab setting}
 
\section{Introduction}
The findings of the previous studies will have shown the influence of IAC on people's strategies of managing looking up information and how different strategies may be more efficient and accurate than others. 
...
The design recommendations will take into account both findings from Study 4 on what influences people's strategies and what is desirable, as well as the setting studied in Study 1 and 3 and what is feasible. For example, desired changes in the actual interface may be too expensive to be realistic, and it may be more feasible to change the way information sources are designed, or how these are laid out in the user's environment. Screenshots of the current interface system will be used (initial ones were obtained in Study 1, additional ones will be obtained in Study 3). 

\subsection{Aim}
Study 5 aims to evaluate different design recommendations in a controlled experiment. It will use the same task paradigm as Study 4, but change some aspects of the task, to see if these changes have an influence on the strategies people adopt in looking up information for a data entry task.
and whether these changes can make people adopt strategies that improve accuracy. 


\subsection{Hypotheses}
\begin{itemize}
\item   
Visually grouping low-IAC items and high-IAC items in the entry system can support users in minimising switch costs between entering and looking up information. 
\item
Providing shortcuts for entering frequently used information can reduce switches needed between entering and looking up information. 
\end{itemize}
...
\chapter{Study 6: evaluating design changes in a financial office setting}
 
\section{Introduction}
Study 5 will have tested the design recommendations in a controlled environment and will have given insight if changing certain design features influences people's strategies, speed and accuracy.  
...
\begin{itemize}
\item
Demonstrate how the design recommendations could be implemented in the financial office setting in which the expenses task is currently done. 
\item
Demonstrate how the design recommendations can influence people's strategies in entering expenses in a financial office setting.
\end{itemize}
}
